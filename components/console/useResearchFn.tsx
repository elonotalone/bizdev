  ...
  const restoreSessionSnapshot = useCallback(
    (snapshot: Record<string, unknown>) => {
      if (
        ...
        snapshot.engine !== "research"
      ) {
        return;
      }
      const nextMaterial =
        typeof snapshot.material === "string" ? snapshot.material : "";
      setCompany(typeof snapshot.company === "string" ? snapshot.company : "");
      setWebsite(typeof snapshot.website === "string" ? snapshot.website : "");
      setMaterial(nextMaterial);
      setMaterialTemplate(
        restoredTemplate(snapshot.materialTemplate, nextMaterial),
      );
      setFocus(typeof snapshot.focus === "string" ? snapshot.focus : "");
      setReport(typeof snapshot.report === "string" ? snapshot.report : "");
      setOpen(
        snapshot.open === "basic" ||
          snapshot.open === "material" ||
          snapshot.open === "focus" ||
          snapshot.open === null
          ? snapshot.open
          : "material",
