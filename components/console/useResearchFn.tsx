...
export function useResearchFn(onNeedAuth: () => void): {
  ...
  const { user } = useUser();
  const [open, setOpen] = useState<"basic" | "material" | "focus" | null>("basic");
...
${material.trim() || "（未提供更多素材，请基于公司名与行业常识给出框架性分析并标注需补充项）"}`,
    ...
    <div className="space-y-3">
      ...
      >
        <textarea
          ...
          placeholder="把目标公司的官网 About/产品页文案、公司简介、领英资料、往来邮件等粘贴到这里，AI 基于这些做分析。"
          value={material}
      ...
      content: (
        <div className="flex h-full flex-col">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-sm font-bold text-stone-800">调研报告</h2>
            <button
              type="button"
              onClick={() => void copy()}
              disabled={!report}
          ...
          {busy && !report ? (
            ...
            <CanvasEmpty
              title="调研报告会显示在这里"
              hint="填写公司、粘贴资料，点「生成调研报告」后即可在此查看、复制。"
            />
          )}
        </div>
      ),
  ...
  const applyPatch = (patch: OpsPatch) => {
    ...
    if (typeof s.material === "string") setMaterial(s.material);
