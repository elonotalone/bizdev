...
import { StudioSection, CanvasEmpty, Markdown, OptionRow, type CanvasTab } from "@oceanleo/ui/shell";
      ...
      >
        <div className="space-y-3">
          <OptionRow
            ...
            value={scene}
            onChange={(v) => setScene(v ?? DEFAULT_SCENE)}
          />
          <OptionRow
            accent={ACCENT}
            options={LANGS.map((l) => ({ value: l, label: l }))}
            value={lang}
            onChange={(v) => setLang(v ?? DEFAULT_LANG)}
          />
        </div>
      </StudioSection>
    </div>
          ...
          {busy && !result ? (
            <div className="grid flex-1 place-items-center py-16">
              <div className="text-center">
                ...
                <p className="mt-3 text-xs text-stone-400">{tt("正在撰写 {lang} 开发信…", { lang })}</p>
              </div>
            </div>
          ) : result ? (
